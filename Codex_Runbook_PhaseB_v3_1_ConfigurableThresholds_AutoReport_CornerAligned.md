# Codex Runbook（Phase B v3.1）：可配置门禁 + Baseline 自适应 + 自动报告（Corner Feasibility 对齐版）

> **单文件真源（Single Source of Truth）**：请只以本文件为准。任何旧版本（包括 v3 与独立补丁文件）均视为已合并且不再单独执行。  
> v3.1 的唯一目的：把 Phase A 补丁 #2（Corner Feasibility / v_ratio_cap_ang）纳入 Phase B 的 baseline、门禁、动作有效性检查与报告产物，避免“对不齐导致评估失真”。


### 0.2（v3.1 追加）Phase A 补丁 #2 Corner Feasibility 通过证明
进入 Phase B 前，必须额外满足（Square 专家策略）：
- `reached_target == True`
- `stall_triggered == False`
- `max_abs_e_n <= 0.98 * half_epsilon`
- `mean_speed_util_in_band >= 0.70`（或按你系统尺度在 gates.yaml 中配置，但不能缺失）

---

## v3.1 强制覆盖条款（必须先读）
以下条款在本文件中已**内嵌**到对应章节；即使你只看这一段也能正确执行：

1) **Baseline 必须重建**：任何 Phase B 训练/门禁前，先用 **Phase A 补丁 #2 后的专家策略**生成 baseline（Square 必须到终点且 `max_abs_e_n <= 0.98*half_epsilon`），否则禁止进入 Phase B。  
2) **动作有效性检查升级为两段式**：  
   - 直线段：v_action 对 v_exec 的单调性（可加速）  
   - 拐角段：无论 v_action 多大，`v_exec` 都必须被 `v_cap_final` 硬裁剪住（cap 生效）  
3) **Stage2（Square）门禁新增拐角可行性指标**：  
   - `max_abs_e_n <= 0.98*half_epsilon`（margin）  
   - `cap_ang_active_ratio >= 0.05`（回归定位用，阈值可配）  
4) **产物/报告增强**：新增 `v_cap_breakdown.png`（brake/ang/final 三曲线）并在 report.md 中展示与诊断（cap 谁在主导）。
...
> v3.1：动作有效性检查必须是“两段式”，确保**可加速**且**拐角会被 cap 硬裁剪**（否则训练必翻车）。

脚本行为（不训练）：

### 段 1：直线段单调性（能加速）
1) 固定 seed reset 到同一初始状态（直线段）。  
2) 在同一 state 下对比 3 个动作（`omega_ratio=0`）：  
   - A: `v_ratio=0.2`  
   - B: `v_ratio=0.5`  
   - C: `v_ratio=0.8`  
3) 运行 1 步，记录 `v_ratio_exec`（或 v_exec）。

必须断言：
- `v_ratio_exec(C) > v_ratio_exec(B) > v_ratio_exec(A)`（严格单调）
- `v_ratio_exec(C) - v_ratio_exec(A) >= gates.action_effectiveness.delta_min_v_ratio_exec`

### 段 2：拐角段 cap 裁剪生效性（关键新增）
1) reset 后推进到“接近拐角”的状态（以 env 的 `dist_to_turn`/`turn_angle` 判定）：  
   - `dist_to_turn <= gates.stage2_square.turn_dist_threshold`  
   - `abs(turn_angle) >= gates.common.turn_angle_min_rad`（推荐 pi/4）
2) 对同样三组动作（omega 取 0 或专家 omega，但必须一致）执行 1 步，记录：  
   - `v_ratio_exec`  
   - `v_ratio_cap_final`（即 env 实际使用的 cap）  
3) 必须断言（容差从 gates 读取）：  
   - `v_ratio_exec <= v_ratio_cap_final + gates.common.cap_exec_tol`

失败时必须打印：
- dist_to_turn, turn_angle  
- v_ratio_cap_brake / v_ratio_cap_ang / v_ratio_cap_final  
- v_ratio_cmd（若可得）与 v_ratio_exec

> 如果段 2 失败：说明拐角 cap 没硬起来，PPO 会学会在 corner 乱加速并越界——禁止进入训练阶段。
...
- （v3.1 新增）max_abs_e_n <= 0.98*half_epsilon（margin，避免擦边虚通过）
- （v3.1 新增）cap_ang_active_ratio >= 0.05（回归定位用，阈值写入 gates.yaml）
...
> **强制声明（v3.1 追加）**：在进入 Phase B 的任何训练/门禁之前，必须先在当前代码版本（包含 Phase A 补丁 #1 与补丁 #2）下运行 baseline 生成脚本。
> baseline 必须满足 Square 专家策略 `reached_target==True` 且 `max_abs_e_n <= 0.98*half_epsilon`。
> 如 baseline 不满足，禁止进入 Phase B（这意味着 Phase A 拐角可行性链路仍不稳定）。

并且 baseline.json（每个 task）必须额外包含：
- `cap_ang_active_ratio`
- `mean_v_ratio_cap_ang`
- `mean_v_ratio_cap_brake`
- `mean_v_ratio_cap_final`
...
7) （v3.1 新增）若存在 `v_cap_breakdown.png`，必须嵌入展示，并在指标表下新增一段“cap 主导诊断”：
   - cap_ang_active_ratio
   - mean_v_ratio_cap_brake/ang/final
   - 失败提示：cap_ang_active_ratio≈0 → 检查 corner wiring；≈1 且 final 很低 → ang cap 过保守
