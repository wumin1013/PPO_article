...
- **直线段**：高进给、贴线（误差不超带）
- **拐角段（Arc-in-band）**：容许带内自动平滑（允许带内自由偏移，不强迫贴中心线），形成更平滑的圆弧/切角，使“转弯更不激烈”且**降速尽量少**
...
- **中心线 = 进度/参考方向**：用于定义 progress 与参考切向；拐角段允许在带内偏离中心线，以换取更平滑的几何形态（圆弧）与更小的降速。
...
- **允差带合规（硬约束）**：`max_abs_contour_error` ≤ `half_epsilon`（拐角段允许带内偏移，不要求贴中心线）
- **直线不退化**：直线/非拐角段贴线不变差（若分段列可用，用 non-corner 段的误差统计；否则以 steps/进度等 proxy 防退化）
...
- **降速更少**：corner_mask 段 min(v) 更高 / v_drop_ratio 更高（或等价指标更优）
- **圆弧化（几何层面）**：拐角处轨迹形态从“贴中心线拐折”转为“允差带内圆弧/切角”（以 overlay/trajectory_points 可视化为准）
