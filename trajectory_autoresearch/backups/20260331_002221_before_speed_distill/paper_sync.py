...
def _rollout_trace(config: dict[str, Any], path_cfg: dict[str, Any], model_path: Path, *, retries: int = 1) -> dict[str, Any]:
    ...
    for attempt_idx in range(max(1, int(retries))):
        ...
        while not done:
            ...
            state = next_state
            if progress_now >= float(COMPLETION_PROGRESS_THRESHOLD):
...
def _rollout_completed(row: dict) -> bool:
    ...
    done_reason = str(row.get("done_reason", "")).strip().lower()
    return bool(progress >= float(COMPLETION_PROGRESS_THRESHOLD) or done_reason == "success")
    ...
    if isinstance(eval_payload, dict):
        ...
        if isinstance(path_results, dict) and path_results:
            ...
            for path_name, row in path_results.items():
                ...
                done_like = bool(progress >= float(COMPLETION_PROGRESS_THRESHOLD))
...
def _build_appendix_tex(current_best: dict, suite_variants: dict[str, dict]) -> str:
    ...
    lines = [
        ...
        "",
        r"\subsection{搜索空间}",
        "当前自动搜索并不直接修改 PPO 主体结构，而是在固定评测协议下围绕有限且可解释的候选配置族进行探索。搜索空间主要包括：进度/时间压强类、转角释放与平滑权衡类、学习型前瞻恢复类以及训练动态调节类。各候选仅对已有奖励项、前瞻控制项和 PPO 超参数做小范围增量调整，以保证搜索过程可复现且便于分析。",
        "",
        r"\subsection{选择准则}",
        "当前自治流程采用两阶段筛选。第一阶段使用较低成本的路径子集与 episode 数进行粗筛，重点关注完成路径数、成功率与稳定性；第二阶段仅对入围候选进行全路径复评估，并将平均完成时间显式纳入综合得分。候选的晋升规则为：先比较完成路径数，再比较包含完成时间项的综合得分；若得分接近，则继续比较平均完成时间、成功率、平均进度与停滞率。",
        "",
    ...
    if latest_status:
        lines.append(
            "当前仍在运行的长时搜索任务编号为 "
            f"{_latex_mono(str(latest_status.get('run_id', '')))}，"
            f"状态为 {_latex_mono(str(latest_status.get('status', 'unknown')))}，"
            f"预计结束时间为 {_latex_mono(str(latest_status.get('deadline_time', '待更新')))}。"
        )
        lines.append("")
    ...
    if top_rows:
        ...
        for row in top_rows:
            lines.append(
                rf"{_latex_mono(str(row.get('experiment_id', '')))}"
                rf" & {_latex_mono(str(row.get('candidate', '')))}"
