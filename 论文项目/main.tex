...
Traditional CNC motion planning is usually built around a serial pipeline: geometric smoothing is performed first, and feedrate planning is then applied to the smoothed path~\cite{Sun2022,Yan2023}. This design improves the continuity of discrete tool paths and is straightforward to implement. Its limitation is that the path geometry has already been chosen before feedrate planning starts. The feedrate planner can mainly accelerate or decelerate along the prescribed trajectory, but it cannot readily use the spatial freedom inside the contour-tolerance band to modify how the tool passes through corners or curvature-transition regions. Consequently, geometric continuity, machining efficiency, and velocity, acceleration, and jerk constraints are often handled in separate steps, even though they are tightly coupled in high-speed, high-precision machining.

For CAM-generated line--arc tool paths, the serial pipeline is mainly implemented through local smoothing, global smoothing, and feedrate planning. Local smoothing builds transition curves between adjacent segments and is suitable for real-time interpolation because the computation is local and the contour error can be controlled explicitly~\cite{Yan2023,Hu2024,Li2020}. Its limitation is that short segments and consecutive corners can cause adjacent transitions to overlap or interfere, reducing both smoothing quality and feedrate improvement~\cite{Li2020}. Global smoothing reconstructs the path over a larger domain and can improve continuity for dense short segments, but curve fitting, global reconstruction, or iterative optimization may increase computation, weaken local feature preservation, and make real-time use more difficult under strict contour-error constraints~\cite{Li2020}. Feedrate planning enforces velocity, acceleration, and jerk limits along a prescribed geometric path~\cite{Sun2022,Wu2022,Lee2011,Erkorkmaz2001,Song2020}. Because the path has usually been fixed before feedrate planning, however, it mainly changes the speed along that path and has limited ability to reshape the executed trajectory inside the tolerance band~\cite{Zhang2021,Zhang2026,WangAIM2025}. Moreover, look-ahead studies show that upcoming geometry affects both feedrate planning and local geometric response, while a fixed preview range adapts poorly to different stages such as straight-line feed, corner approach, corner traversal, and corner exit~\cite{Song2020,Zhong2025}. These limitations motivate a closed-loop motion-planning framework that generates tolerance-band trajectories and kinematically feasible feedrates in a unified decision process.

A more direct line of work tries to reduce the separation between geometric smoothing and feedrate planning by determining motion behavior in one step. Such methods can couple trajectory generation with speed regulation and can use the tolerance band more actively in corner and curvature-transition regions. Optimization-based one-step methods, however, often involve nonlinear constraints, iterative solvers, or simplified local models, which can limit their real-time applicability. Learning-based direct planning provides another route. Li et al. were among the first to apply reinforcement learning to CNC trajectory smoothing and proposed the neural-network numerical control (NNC) direct trajectory smoothing method~\cite{Li2020}. NNC formulates trajectory smoothing as an action-decision problem. At each interpolation cycle, a neural network outputs servo control commands from the current tool path and operating state. The action contains angle-change and step-length-change components, which are converted into a position command. The reward is mainly defined by the contour error from the current point to the G-code path. The reported experiments show that NNC improves machining efficiency over local smoothing methods and improves contour accuracy over global smoothing methods. The remaining challenge is that direct neural control still needs explicit constraint handling when used for CNC motion planning, since the output of the neural network does not automatically satisfy velocity, acceleration, and jerk limits.

The same concern also appears in broader studies on learning-based control and safe action generation. Beyond NNC, Samsonov et al.~\cite{Samsonov2023} studied CNC manufacturing decision problems using deep representation learning and reinforcement learning. Alizadeh Kolagar et al.~\cite{Alizadeh2025} combined deep reinforcement learning with a jerk-bounded trajectory generator to improve kinematic feasibility in learning-based control. Work on safe learning-based control, including OptLayer proposed by Pham et al.~\cite{Pham2018}, the shielding approach of Alshiekh et al.~\cite{Alshiekh2018}, control-barrier-function-based safety control developed by Cheng et al.~\cite{Cheng2019}, and the predictive safety filter introduced by Wabersich and Zeilinger~\cite{Wabersich2021}, shows that learned actions often need projection or filtering mechanisms before they can satisfy explicit constraints.
...
This paper proposes a reinforcement learning-based CNC motion-planning framework with jerk-constrained action projection and adaptive preview as J-NNC (Jerk-constrained Neural Numerical Controller). The proposed framework formulates motion planning for mixed tool paths as a constrained sequential decision-making problem within the tolerance band. A kinematic constraint module (KCM) is placed between the learned policy and the execution layer. It maps unconstrained steering and feedrate intentions to interpolation control variables that satisfy velocity, acceleration, and jerk constraints. The policy outputs steering, feedrate, and look-ahead-distance adjustment actions in a closed loop. The state representation includes corner information and multi-scale forward geometric information, and the reward considers path progress, contour error, direction consistency, and motion smoothness. \figref{fig:comparison_arch} compares the conventional serial motion-planning framework with the closed-loop J-NNC motion-planning framework.
...
The paper makes three main contributions. It introduces a KCM between the learned policy and the execution layer. The KCM maps raw steering and feedrate outputs into executable interpolation commands while enforcing linear and angular velocity, acceleration, and jerk limits, thereby improving the kinematic feasibility of learning-based CNC motion planning. It also develops a closed-loop tolerance-band decision formulation for CNC motion planning. The structured state encodes tracking, kinematic, corner, and multi-scale forward-geometric information, while the steering, feedrate, and preview-distance actions allow the policy to adjust local motion behavior and geometric perception across different path stages. Finally, experiments on square, circular, and butterfly-shaped paths show that J-NNC preserves full path progress and eliminates the linear-jerk violations observed in the NNC baseline and the no-KCM ablation.
