...
J-NNC通过周期性的神经网络推理，实时生成伺服指令（如图1b所示），因此被定义为一种“直接”运动规划方法。在此框架下，神经网络作为一个智能控制单元，替代了传统CNC系统中独立的前瞻(Look-ahead)、平滑(Smoothing)和轨迹规划等多个模块。与NNC相比，J-NNC通过内置的捷度控制，确保输出的伺服指令直接满足运动学约束，从而无需后续的速度规划环节，实现了更彻底的端到端直接控制。
